...
class TestCase(unittest.TestCase):
    ...
    def test_multivariate(self):
        ...
        print("flux", zeroth_moment/9999.96)
    ...
    def test_multivariate_v2(self):
        ...
        self.assertAlmostEqual(covariance_matric[1][1], covariance_matrix[1][1], places=5)
        self.assertAlmostEqual(covariance_matric[0][1], covariance_matrix[0][1], places=5)
        #passes
    ...
    def test_null_deblend(self):
        ...
        print('flux; ', flux)
        print('com_x: ', first_moment_x)
        print('com_y: ', first_moment_y)
        print('covariance_matrix: ', covariance_matrix)
        print('temp image', returned_image)
        x_com, y_com = first_moment_x, first_moment_y
        center_of_mass = [x_com, y_com]
        rv = scipy.stats.multivariate_normal([x_com, y_com], covariance_matrix)
        x, y = np.mgrid[x_min:x_max:distance, y_min:y_max:distance]
        pos = np.dstack((x, y))
        number_of_rows = int((x_max-x_min)/distance)
        number_of_columns = int((y_max-y_min)/distance)
        new_image = [[0]*number_of_rows for _ in range(number_of_columns)]
        new_image = new_image + rv.pdf(pos)*flux
        print('new_image: ', new_image)
        flux, first_moment_x, first_moment_y, covariance_matrix = desc.slrealizer.null_deblend(new_image)
        print('flux; ', flux)
        print('com_x: ', first_moment_x)
        print('com_y: ', first_moment_y)
        print('covariance_matrix: ', covariance_matrix)
    ...
    def test_multivariate_v2(self):
        ...
        np.testing.assert_array_almost_equal(moment_matrix1, moment_matrix2)
        return
